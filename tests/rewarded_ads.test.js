...
const viewportCss = fs.readFileSync('viewport-scale.css', 'utf8');
assert.match(viewportCss, /html\.gd-ad-active body\s*\{[^}]*transform:\s*none\s*!important/s,
  'the design-surface transform must not scale the SDK ad');
assert.match(viewportCss, /#gdsdk__advertisement[\s\S]*?width:\s*100vw\s*!important[\s\S]*?height:\s*100vh\s*!important/,
  'the SDK ad surface must cover the real viewport');
const code = `${source}
renderShopTable = () => {};
